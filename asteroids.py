...
class GameManager:
    ...
    def generate_level(self):
        ...
        [self.add_sprite(Enemy(None, 2, {}, TURRET_IMAGE,
                               ...
                               random.randint(70, 300),
                               health=random.randint(3, 10))) for _ in range(enemyNum)]
...
class Explosion(Object):
    ...
    def draw_sprite(self, screen):
        ...
        screen.blit(self.surface.convert_alpha(), (self.pos[0] - self.frame_width / 2, self.pos[1] - self.frame_height / 2))
...
class Player(Object):
    ...
    def update(self, game_manager):
        ...
        # updates angle based on mouse location
        old_angle = self.angle

        xdiff = pygame.mouse.get_pos()[0] - self.pos[0]
        ydiff = pygame.mouse.get_pos()[1] - self.pos[1]

        self.angle = math.degrees(math.atan2(-ydiff, xdiff))

        if not self.angle == old_angle:
    ...
    def draw(self, screen):
        ...
        for i in range(self.health):
            hor = i * HEART_IMAGE.get_width() * 0.7

            screen.blit(HEART_IMAGE, (hor % SCREEN_SIZE[0] + 10, 10 + 6 * (hor // SCREEN_SIZE[0])))

    def ShootyShoot(self, game_manager):
        spread_range = self.shoot_tick * 100
        if spread_range > 15000:
            spread_range = 15000
        spread = random.randint(-spread_range, spread_range) / 1000

        game_manager.add_sprite(Bullet(1, {"PBullet"}, BULLET_IMAGE, copy.copy(self.pos),
                                    [self.velocity[0] + math.cos(math.radians(self.angle + spread)) * 23,
                                     self.velocity[1] + -math.sin(math.radians(self.angle + spread)) * 23]))

    def takeDamage(self, game_manager, pos):
        if self.damageCooldown == 0:
            self.health -= 1
            game_manager.add_sprite(Explosion(-1, 5, {}, (8, 1), 5, EXPLODE_IMAGE, copy.copy(pos), 8))

            sound = pygame.mixer.Sound("Explode1.wav")
            sound.set_volume(.01)
            sound.play()
    ...
    def collideEnemy(self, sprite, game_manager):
        ...
        if d_check <= self.rect.width / 2.1 + sprite.rect.width / 2.1:
            sprite.takeDamage(game_manager, sprite.pos)
            self.takeDamage(game_manager, self.pos)
            if self.damageCooldown == 0:
    ...
    def collideBullet(self, sprite, game_manager):
        d_check = DistanceCalc(self.pos, sprite.pos)
        if d_check <= self.rect.width / 2.1 + sprite.rect.width / 2.1:
            self.takeDamage(game_manager, sprite.pos)
...
class Enemy(Object):
    ...
    def update(self, game_manager):
        ...
        ply = [sprite for sprite in game_manager.SPRITES if "player" in sprite.tags]
        if len(ply):
            ply = ply[0]

            x_diff = ply.pos[0] - self.pos[0]
            y_diff = ply.pos[1] - self.pos[1]
...
EXPLODE_IMAGE = pygame.transform.smoothscale(pygame.image.load("explosion.png"), cscale(800, 100)).convert_alpha()
BULLET_IMAGE = pygame.transform.flip(pygame.transform.smoothscale(pygame.image.load("bullet.png"), cscale(30, 5)), True, False).convert_alpha()
SHIP_IMAGE = pygame.transform.smoothscale(pygame.image.load("ship.png"), cscale(100, 80)).convert_alpha()
TURRET_IMAGE = pygame.transform.rotate(pygame.transform.smoothscale(pygame.image.load("alienturret.png"), cscale(80, 70)), -90).convert_alpha()
HEART_IMAGE = pygame.transform.smoothscale(pygame.image.load("heart.png"), cscale(15, 15)).convert_alpha()
HEART_TRANS_IMAGE = copy.copy(pygame.transform.scale(HEART_IMAGE, cscale(10, 10))).convert_alpha()
